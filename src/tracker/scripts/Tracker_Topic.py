...
def detect_and_track(data, args):
    with torch.no_grad():
        # Run inference
        t0 = time.time()
...
def main():
    ...
    rospy.Subscriber('camera/rgb/image_raw', Image, image_callback, opt)
    
    while not rospy.is_shutdown():
        if opt["image_flag"]:
            # frame = cv2.cvtColor(opt["image"], cv2.COLOR_BayerBG2BGR)  # convert to RGB
            frame = opt["image"]
