...
from common import callback, load_param_gmsl2_3, detect_and_track_gmsl2_3
...
def main():
    ...
    opt = load_param_gmsl2_3()
    opt["image_flag"] = False
    opt["image"] = Image()

    rospy.Subscriber('Traffic_Lights_Num', Int8, callback, opt, queue_size=1)
    rospy.Subscriber('camera/rgb/image_gmsl2', Image, image_callback, opt, queue_size=1)
    ...
    while not rospy.is_shutdown():
        if opt["image_flag"]:
            ...
            if opt['flag']:
                detect_and_track_gmsl2_3(frame, opt)
            else:
